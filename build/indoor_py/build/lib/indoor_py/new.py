...
class ApartmentExplorer(Node):
    ...
    def control_loop(self):
        ...
        if self.gps_data and self.start_position:
            ...
            # Dynamic threshold: increase if front space is clear
            dynamic_threshold = 0.3 if front_lidar_min < 1.0 else 0.5

            if distance_moved < dynamic_threshold:
                if current_time - self.recovery_timer >= 10:  # 10-second timeout
    ...
    def explore_room(self):
        """Methodically explore a room by moving in a systematic pattern with a timeout."""
        ...
        for _ in range(4):  # Make 4 passes in the room
            # Break out of exploration if timeout occurs
            if time.time() - exploration_start_time > 15:  # 15-second timeout
                self.get_logger().info("Exploration timeout reached. Exiting room.")
                break
            ...
            # Turn to cover another section of the room
    ...
    def detect_room_entry(self):
        """Detect if the robot is entering a room based on LIDAR readings."""
        left_clear = min(self.scan_cleaned[LEFT_SIDE_INDEX:LEFT_FRONT_INDEX]) > SAFE_STOP_DISTANCE
        right_clear = min(self.scan_cleaned[RIGHT_FRONT_INDEX:RIGHT_SIDE_INDEX]) > SAFE_STOP_DISTANCE
        front_clear = min(self.scan_cleaned[LEFT_FRONT_INDEX:RIGHT_FRONT_INDEX]) > SAFE_STOP_DISTANCE

        # Detect room-like conditions: front is mostly clear, and sides are confined
        room_condition = front_clear and (not left_clear or not right_clear)

        # Debugging logs to confirm room detection
        self.get_logger().info(f"Room detection - Front: {front_clear}, Left: {left_clear}, Right: {right_clear}")

        return room_condition
    
    def leave_room(self):
        """Guide the robot to leave the room and return to an unexplored area."""
        self.get_logger().info("Exiting room systematically.")

        # Backup slightly to clear obstacles near the entrance
        self.cmd.linear.x = -LINEAR_VEL
        self.cmd.angular.z = 0.0
        self.publisher_.publish(self.cmd)
        time.sleep(1)
        ...
        left_clear = min(self.scan_cleaned[LEFT_SIDE_INDEX:LEFT_FRONT_INDEX])
        right_clear = min(self.scan_cleaned[RIGHT_FRONT_INDEX:RIGHT_SIDE_INDEX])

        if left_clear > right_clear:
            self.cmd.angular.z = 1.0  # Turn left
        else:
            self.cmd.angular.z = -1.0  # Turn right

        self.publisher_.publish(self.cmd)
        time.sleep(2)  # Allow time to turn
